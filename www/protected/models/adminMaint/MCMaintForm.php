<?php
class UserAccountMaintForm extends McMaster {
	public $reEnterPW;
	...
	public function edit() {
		if ($this->validate()) {
			if (empty($this->password)) {
				return $this->save(false, array('role', 'sts', 'modify_by', 'modify_date'));
			}
			else {
				// Password is entered
				if ($this->password == $this->reEnterPW) {
					return $this->save(false, array('password', 'role', 'sts', 'modify_by', 'modify_date'));
				}
				else {
					$this->addError('password', 'Unmatch');
					$this->addError('reEnterPW', 'Unmatch');
