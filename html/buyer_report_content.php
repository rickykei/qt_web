 <div id="content">
	
	<div id="leftcol"><img src="/images/report_logo.png"></div>
	
	<div id="rightcol">
	
		
		<ul>
		 
		<li class="row">
			<img class="rowpic" src="/images/supplier_search.png" width="80"/>
			<form id="checklist_search" action="" >
			<label>Req Report CD</label><input type="text" /><br/>
			<label>Supplier Area</label><input type="text" /><br/>
			<label>Supplier Code</label><input type="text" /><br/>
			<label>Supplier Name</label><input type="text" /><br/>
			<label>Supplier Industry</label><input type="text" /><br/>
			<label>QAuditor</label><input type="text" /><br/>
			<label>Risk Level</label><input type="text" /><br/>
			<label>Sch Start Date</label><input type="text" /><br/>
			<label>Sch End Date</label><input type="text" /> 
			<input class="button" type="submit" id="createchecklistbtn" value="search" class="button"/>
			</form>
			 
		</li>
		</ul>
		<div class="clear"></div>
	
		<div id="contentresult">
		<table id="checklist_table">
			<tr class="checklist_table_header">
  			 <th width="180">Supplier Name</th>
			 <th width="250">Report No</th>
				<th width="150">Industry</th>
				<th width="100">Area</th>
				<th width="150">Risk LV</th>
				<th width="150">Auditor </th>
				<th width="150">Audit start date</th>
				<th width="150">Audit end date</th>
				<th width="30">PDF</th>
				
				 
			</tr>
			
			<tr class="checklist_table_content">
				<td>Angel Plastic</td>
				<td>APM-020410-001</td>
				<td>Manufactur</td>
				<td>Shanghai</td>
				 <td>high risk</td>
				 <td>Ricky Wong</td>
				 <td>2011-02-23</td>
				 <td>2011-02-23</td>
				<td>PDF</td>
				 
			</tr>
			
			<tr class="checklist_table_content">
				<td>Greenland muolding</td>
				<td>APM-020410-001</td>
					<td>Manfacturing</td>
			<td>Shanghai</td>
			 <td>high risk</td>
				 <td>Ricky Wong</td>
			<td>2011-02-23</td>
				<td>2011-02-23</td>
				 
					<td>PDF</td>
			 
			 
			</tr>
		</table>	
		</div>
	 
	</div>

	
	
</div>